          ...
          {#each weddings as w (w.id)}
            ...
            <div class="group">
              <div
                ...
                onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') selectWedding(w); }}
                role="button"
                tabindex="0"
                aria-disabled={selecting !== null}
                class="w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left {selecting !== null ? 'opacity-50 pointer-events-none' : ''} {isSelected ? 'border-gold bg-gold-50/50' : 'border-gray-200 hover:border-deep-red hover:bg-red-50'}"
              >
