<script lang="ts">
  ...
  onMount(() => {
    ...
    if (tableParam) {
      ...
      if (!isNaN(tableId) && TABLE_DEFINITIONS.some(t => t.id === tableId)) {
  ...
  let selectedTable = $derived(selectedTableId ? TABLE_DEFINITIONS.find(t => t.id === selectedTableId) ?? null : null);
