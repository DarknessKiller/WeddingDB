...
const DEFAULT_TABLES: BanquetTable[] = [
...
// localStorage key
const STORAGE_KEY = 'weddingdb_tables';

// Load tables from localStorage or use defaults
let _tables: BanquetTable[] = [];

function loadTables(): BanquetTable[] {
...
function saveTables(tables: BanquetTable[]) {
...
// Export getter for components to use (always returns current state)
export function getTableDefinitions(): BanquetTable[] {
  return _tables;
}

// Export setter for settings page
export function setTableDefinitions(tables: BanquetTable[]) {
  _tables = tables;
  saveTables(tables);
}

// Re-export for backward compatibility — but use getTableDefinitions() for dynamic access
export const TABLE_DEFINITIONS: BanquetTable[] = _tables;
